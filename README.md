...
A persistent browser dashboard that shows your live agent activity, installed skills, an integrated terminal, and controls for secrets, MCP servers, and effort level. Launches automatically on install. Run `ecc cockpit` to reopen it anytime. See [docs/command-deck.md](docs/command-deck.md) for full usage.
...
Everything in Pro, plus a complete algotrading development workspace deployed to `~/QuantLab`: backtest + walk-forward + Monte Carlo validation suite, ML labeling/training pipeline, risk engine with execution safeguards (paper-first), LLM trade-review layer, a worked example strategy wired end-to-end, and the shipped test suite. `ecc quantlab --setup`, then `ecc quantlab --doctor`. Tools, not signals.
...
AgentForge is a one-time purchase (Starter $39, Builder $99, Pro $249, Quant Lab $499) with 12 months of free updates. Licenses are issued via [agentforge.army](https://agentforge.army/pricing).
